...
static lv_obj_t *bottom_bar(lv_obj_t *parent)
{
    lv_obj_t *cont;
    lv_obj_t *btn;
...
void lv_menu(lv_obj_t *parent)
{
    ...
    //lv_list_set_anim_time(list1, 50);
    ...
    list_btn = lv_list_add_btn(list1, &group_80x80, "group select");
    lv_obj_set_event_cb(list_btn, event_handler);

    list_btn = lv_list_add_btn(list1, &group_80x80, "member select");
    lv_obj_set_event_cb(list_btn, event_handler);

    list_btn = lv_list_add_btn(list1, &group_80x80, "friend select");
    lv_obj_set_event_cb(list_btn, event_handler);

    list_btn = lv_list_add_btn(list1, &group_80x80, "GPS");
    lv_obj_set_event_cb(list_btn, event_handler);

    list_btn = lv_list_add_btn(list1, &group_80x80, "record");
    lv_obj_set_event_cb(list_btn, event_handler);

    list_btn = lv_list_add_btn(list1, &group_80x80, "POC settings");
    lv_obj_set_event_cb(list_btn, event_handler);
